...
long nLastXmitTimeStamp = nPgmTime;//create a timestamp for bleutooth
long nDeltaTime         = 0;//dont know

const int kMaxSizeOfMessage = 30;//create a variable for the maximum message size
const int INBOX = 5;//dont know
...
int bleutooth_control(void){
	...
	*/
	TFileIOResult nBTCmdRdErrorStatus;
  int nSizeOfMessage;//i dont know
  ubyte nRcvBuffer[kMaxSizeOfMessage];
  int stopcode = 0;
  ...
  nSizeOfMessage = cCmdMessageGetSize(INBOX);
	...
	if (nSizeOfMessage > 0){//Only if an actual message is being received the bleutooth take over system should activate
    	...
    	while (s != "A"){//if A is pressed the robot will resume its duty the normal way
    		...
    		else {//i dont know //not actually nessecary
    			setMultipleMotors(0, motorA, motorB);
    		}
    		if (s == "C"){//if the C button is pressed the loop stops and the stopcode == 1 to stop de code if neccesairy i dont know
    			stopcode = 1;
    			break;
    		}
    	}
...
task main()
{
	...
	*/
	...
	//read the beginvalues of the light sensors i dont know
	int begin_value_s1 = SensorValue[S1];
	int begin_value_s2 = SensorValue[S2];

	//delayed startup i dont know
	...
	startTask (music);//start music as a different task to make sure it runs at the same time as the main task i dont know
	...
	while(1){
		...
		need_for_speed(begin_value_s1, begin_value_s2);//check if the line is curving and if not then just drive
